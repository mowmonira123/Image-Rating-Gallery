...
<div class="row row-cols-4 row-cols-md-3 row-cols-sm-1 row-cols-lg-4 py-2">
	<?php 
		$qry = $conn->query("SELECT * FROM `images` where `delete_f` = 0 and album_id = '{$_GET['id']}' order by `rate` desc ");
        ...
        // Rating
        $query = "SELECT ROUND(AVG(rate), 1) as numRating FROM images WHERE id=".$product_id;
...
<script type='text/javascript'>
	...
	$(document).ready(function(){
		...
		$('#add-new').click(function(){
			uni_modal("<i class='fa fa-upload'></i> Add New Image", "albums/manage_image.php?album_id=<?php echo $_GET['id'] ?>")
		})
